...
def test_stream_slices_for_substream(stream_config, stream_api, requests_mock):
    ...
    requests_mock.register_uri(
        "GET", stream.path() + f"/{job_id}", json=JobInfoResponseBuilder().with_id(job_id).with_state("JobComplete").get_response()
    )
    requests_mock.register_uri(
        "GET",
