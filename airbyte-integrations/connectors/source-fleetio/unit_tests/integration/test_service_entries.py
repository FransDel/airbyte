...
_RECORDS_PATH = FieldPath("service_entries")
_API_URL = "https://secure.fleetio.com/api"
_API_VERSION = "2024-03-15"


def _a_service_entry() -> RecordBuilder:
...
def _service_entries_response() -> HttpResponseBuilder:
...
class ServiceEntriesTest(TestCase):
    @HttpMocker()
    def test_request_is_created_properly(self, http_mocker: HttpMocker):
        http_mocker.get(
            HttpRequest(
                url=f"{_API_URL}/v2/service_entries",
                query_params={
                    "per_page": "100",
                },
                headers={
            ...
            _service_entries_response().build(),
        )
