...
def _a_vehicle_assigment() -> RecordBuilder:
...
def _vehicle_assignments_response() -> HttpResponseBuilder:
...
class VehicleAssignmentsTest(TestCase):
    @HttpMocker()
    def test_request_is_created_properly(self, http_mocker: HttpMocker):
        http_mocker.get(
            HttpRequest(
                url=f"{_API_URL}/v1/vehicle_assignments",
                query_params={
                    "per_page": "100",
                },
                headers={
