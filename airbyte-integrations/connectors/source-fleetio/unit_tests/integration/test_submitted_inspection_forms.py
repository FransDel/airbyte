...
_STREAM_NAME = "submitted_inspection_forms"
_TEMPLATE_NAME = "submitted_inspection_forms"
_RECORDS_PATH = FieldPath("submitted_inspection_forms")
_API_URL = "https://secure.fleetio.com/api"
_API_VERSION = "2024-03-15"


def _a_submitted_inspection_form() -> RecordBuilder:
...
def _submitted_inspection_forms_response() -> HttpResponseBuilder:
...
class SubmittedInspectionFormsTest(TestCase):
    @HttpMocker()
    def test_request_is_created_properly(self, http_mocker: HttpMocker):
        http_mocker.get(
            HttpRequest(
                url=f"{_API_URL}/v1/submitted_inspection_forms",
                query_params={
                    "per_page": "100",
                },
                headers={
