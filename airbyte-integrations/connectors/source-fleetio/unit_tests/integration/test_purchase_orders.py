...
_TEMPLATE_NAME = "purchase_orders"
_RECORDS_PATH = FieldPath("purchase_orders")
_API_URL = "https://secure.fleetio.com/api"
_API_VERSION = "2024-03-15"


def _a_purchase_order() -> RecordBuilder:
...
def _purchase_orders_response() -> HttpResponseBuilder:
...
class PurchaseOrdersTest(TestCase):
    ...
    def test_request_is_created_properly(self, http_mocker: HttpMocker):
        http_mocker.get(
            HttpRequest(
                url=f"{_API_URL}/v1/purchase_orders",
                query_params={
                    "per_page": "100",
                },
                headers={
            ...
            _purchase_orders_response().build(),
        )
