...
def _airbyte_messages(n: int, batch_size: int, table_name: str) -> Generator[AirbyteMessage, None, None]:
    ...
    for i in range(n):
        if i != 0 and i % batch_size == 0:
            yield AirbyteMessage(type=Type.STATE, state=AirbyteStateMessage(data={"state": str(i // batch_size)}))
        else:
                ...
                record=AirbyteRecordMessage(
                    ...
                    data={
                        ...
                        if random.random() < 0.1
                            ...
                            "dictionary1": {}
                            if random.random() < 0.1
                            else {"key3": fake.first_name(), "key4": "True" if random.random() < 0.5 else True},
                        },
                    }
