...
class ResultBackend(ABC):
    ...
    def write(self, connector: Connector, file_to_persist: FileToPersist) -> None:
        if not file_to_persist.file_content:
            raise ValueError(
                "File content must be set before writing to local directory"
            )
    ...
    def artifact_already_exists(
        self, connector: Connector, file_to_persist: FileToPersist
    ) -> bool:
        raise NotImplementedError(
            "insights_already_exist method must be implemented by subclass"
        )
...
class LocalDir(ResultBackend):
    ...
    def _write(self, connector: Connector, file_to_persist: FileToPersist) -> None:
        ...
        connector_result_directory = (
            self.local_directory / connector.technical_name / connector.version
        )
...
class GCSBucket(ResultBackend):
    ...
    def __init__(
        self, bucket_name: str, key_prefix: str, gcp_project: str = DEFAULT_GCP_PROJECT
    ):
        super().__init__()
    ...
    def _write(self, connector: Connector, file_to_persist: FileToPersist) -> None:
        assert file_to_persist.file_content is not None
        version_blob_prefix = (
            f"{self.key_prefix}/{connector.technical_name}/{connector.version}"
        )
        ...
        for blob_prefix in [version_blob_prefix, latest_blob_prefix]:
            ...
            self.logger.info(
                f"{file_to_persist.file_name} written to {blob.public_url}"
            )
