...
async def upload(
    context: ConnectorContext, gcp_gsm_env_variable_name: str = "GCP_GSM_CREDENTIALS"
) -> Container:
    ...
    def with_secrets_mounted_as_dagger_secrets(container: Container) -> Container:
        if java_log_scrub_pattern_secret:
            ...
            container = container.with_secret_variable(
                "LOG_SCRUB_PATTERN", java_log_scrub_pattern_secret
            )
        container = container.with_exec(
            ["mkdir", "-p", secret_directory_path], skip_entrypoint=True
        )
        for secret in connector_secrets:
            if secret.file_name:
                container = container.with_mounted_secret(
                    f"{secret_directory_path}/{secret.file_name}",
                    secret.as_dagger_secret(context.dagger_client),
                )
        return container
