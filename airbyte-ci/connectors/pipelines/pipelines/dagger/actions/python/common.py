...
from pipelines.dagger.containers.python import (
    with_pip_cache,
    with_poetry_cache,
    with_python_base,
    with_testing_dependencies,
)
...
def with_python_package(
    ...
    """
    package_source_code_directory: Directory = context.get_repo_dir(
        package_source_code_path, exclude=exclude, include=include
    )
    work_dir_path = f"/{package_source_code_path}"
    container = python_environment.with_mounted_directory(
...
async def find_local_dependencies_in_setup_py(python_package: Container) -> List[str]:
    ...
    for line in egg_info_output.split("\n"):
        if line.startswith("writing requirements to"):
            ...
            requirements_txt_content = await with_egg_info.file(
                requires_txt_path
            ).contents()
...
async def find_local_dependencies_in_requirements_txt(
    python_package: Container, package_source_code_path: str
) -> List[str]:
    ...
    Returns:
        List[str]: Paths to the local dependencies relative to the airbyte repo.
    """
    requirements_txt_content = await get_file_contents(
    ...
    for line in requirements_txt_content.split("\n"):
        ...
        if line.startswith("-e .") and line != "-e .":
            local_dependency_path = str(
                (Path(package_source_code_path) / Path(line[3:]))
                .resolve()
...
async def find_local_python_dependencies(
    ...
    container = with_python_package(
        context, python_environment, package_source_code_path
    )
    ...
    if search_dependencies_in_requirements_txt:
        local_dependency_paths += await find_local_dependencies_in_requirements_txt(
            container, package_source_code_path
        )
    ...
    for local_dependency_path in local_dependency_paths:
        ...
        transitive_dependency_paths += await find_local_python_dependencies(
            context, local_dependency_path, True, False
        )

    all_dependency_paths = local_dependency_paths + transitive_dependency_paths
    if all_dependency_paths:
        context.logger.debug(
            f"Found local dependencies for {package_source_code_path}: {all_dependency_paths}"
        )
...
def _install_python_dependencies_from_requirements_txt(
    container: Container,
) -> Container:
...
async def with_installed_python_package(
    ...
    local_dependencies = await find_local_python_dependencies(
        context, package_source_code_path
    )
    ...
    if has_pyproject_toml:
        ...
        container = _install_python_dependencies_from_poetry(
            container, additional_dependency_groups, install_root_package
        )
    elif has_setup_py:
        container = with_pip_cache(container, context.dagger_client)
        container = _install_python_dependencies_from_setup_py(
            container, additional_dependency_groups
        )
...
async def apply_python_development_overrides(
    context: ConnectorContext, connector_container: Container
) -> Container:
    ...
    if context.use_local_cdk:
        ...
        connector_container = connector_container.with_mounted_directory(
            f"/{path_to_cdk}", directory_to_mount
        ).with_exec(
...
def with_pip_packages(
    base_container: Container, packages_to_install: List[str]
) -> Container:
