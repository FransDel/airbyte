...
class SetCDKVersion(Step):
    ...
    async def _run(self) -> StepResult:
        ...
        try:
            ...
            if self.context.connector.language in [
                ConnectorLanguage.PYTHON,
                ConnectorLanguage.LOW_CODE,
            ]:
                updated_connector_dir = (
                    await self.upgrade_cdk_version_for_python_connector(
                        og_connector_dir
                    )
                )
            elif self.context.connector.language is ConnectorLanguage.JAVA:
                updated_connector_dir = (
                    await self.upgrade_cdk_version_for_java_connector(og_connector_dir)
                )
            else:
                return StepResult(
                    step=self,
            ...
            exported_successfully = await diff.export(
                os.path.join(git.get_git_repo_path(), context.connector.code_directory)
            )
    ...
    async def upgrade_cdk_version_for_java_connector(
        self, og_connector_dir: Directory
    ) -> Directory:
        if "build.gradle" not in await og_connector_dir.entries():
            raise ValueError(
                f"Java connector {self.context.connector.technical_name} does not have a build.gradle file."
            )

        build_gradle = og_connector_dir.file("build.gradle")
        build_gradle_content = await build_gradle.contents()

        old_cdk_version_required = re.search(
            r"cdkVersionRequired *= *'(?P<version>[0-9]*\.[0-9]*\.[0-9]*)?'",
        ...
        if self.new_version == "latest":
            new_version = await cdk_helpers.get_latest_java_cdk_version(
                self.context.get_repo_dir()
            )
        else:
            new_version = self.new_version

        updated_build_gradle = build_gradle_content.replace(
            old_cdk_version_required.group("version"), new_version
        )

        use_local_cdk = re.search(r"useLocalCdk *=.*", updated_build_gradle)
        if use_local_cdk is not None:
            updated_build_gradle = updated_build_gradle.replace(
                use_local_cdk.group(), "useLocalCdk = false"
            )

        return og_connector_dir.with_new_file("build.gradle", updated_build_gradle)

    async def upgrade_cdk_version_for_python_connector(
        self, og_connector_dir: Directory
    ) -> Optional[Directory]:
        context = self.context
        og_connector_dir = await context.get_connector_dir()
        ...
        if not pyproject_toml:
            raise ValueError(
                f"Could not find 'pyproject.toml' for {context.connector.technical_name}"
            )
        ...
        if not airbyte_cdk_dependency:
            raise ValueError(
                "Could not find a valid airbyte-cdk dependency in 'pyproject.toml'"
            )
        ...
        updated_connector_dir = og_connector_dir.with_new_file(
            PYPROJECT_FILENAME, updated_pyproject_toml_content
        )

        # Create a new container to run poetry lock
        base_image = self.context.connector.metadata["connectorBuildOptions"][
            "baseImage"
        ]
        ...
        connector_container = base_container.with_mounted_directory(
            "/connector", updated_connector_dir
        ).with_workdir("/connector")

        poetry_lock_file = await connector_container.file(
            POETRY_LOCK_FILENAME
        ).contents()
        updated_container = await connector_container.with_exec(
            ["poetry", "lock", "--no-update"]
        )
        updated_poetry_lock_file = await updated_container.file(
            POETRY_LOCK_FILENAME
        ).contents()

        if poetry_lock_file != updated_poetry_lock_file:
            updated_connector_dir = updated_connector_dir.with_new_file(
                POETRY_LOCK_FILENAME, updated_poetry_lock_file
            )
        else:
    ...
    async with semaphore:
        ...
        async with context:
            ...
            report = ConnectorReport(
                context, steps_results, name="CONNECTOR VERSION CDK UPGRADE RESULTS"
            )
