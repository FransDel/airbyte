...
class BuildConnectorImages(BuildConnectorImagesBase):
    ...
    async def _build_connector(self, platform: Platform, *args: Any) -> Container:
        ...
        if not baseImage:
            raise ValueError(
                "connectorBuildOptions.baseImage is required to build a manifest only connector."
            )
    ...
    def _get_base_container(self, platform: Platform) -> Container:
        ...
        self.logger.info(
            f"Building manifest connector from base image {base_image_name}"
        )
    ...
    async def _build_from_base_image(self, platform: Platform) -> Container:
        ...
        base_container = self._get_base_container(platform).with_file(
            ...
            (await self.context.get_connector_dir(include=[MANIFEST_FILE_PATH])).file(
                MANIFEST_FILE_PATH
            ),
        )

        connector_container = build_customization.apply_airbyte_entrypoint(
            base_container, self.context.connector
        )
