...
class ConnectorTestContext(ConnectorContext):
    def __init__(
        ...
        **kwargs: Any,
        ...
        self.run_step_options = self._skip_metadata_disabled_test_suites(
            self.run_step_options
        )
    ...
    def _handle_missing_secret_store(
        ...
        assert isinstance(
            secret_info["secretStore"], dict
        ), "The secretStore field must be a dict"
    ...
    def _process_secret(
        ...
        if secret_store_alias not in secret_stores:
            ConnectorTestContext._handle_missing_secret_store(
                secret_info, raise_on_missing, logger
            )
            return None
        else:
            ...
            assert isinstance(
                secret_info["name"], str
            ), "The secret name field must be a string"
            if file_name := secret_info.get("fileName"):
                assert isinstance(
                    secret_info["fileName"], str
                ), "The secret fileName must be a string"
                file_name = str(secret_info["fileName"])
            else:
                file_name = None
            return Secret(
                secret_info["name"],
                secret_stores[secret_store_alias],
                file_name=file_name,
            )

    @staticmethod
    def get_secrets_from_connector_test_suites_option(
        connector_test_suites_options: List[
            Dict[str, str | Dict[str, List[Dict[str, str | Dict[str, str]]]]]
        ],
        suite_name: str,
        secret_stores: Dict[str, SecretStore],
        ...
        """
        secrets: List[Secret] = []
        enabled_test_suite = find(
            connector_test_suites_options, lambda x: x["suite"] == suite_name
        )

        if enabled_test_suite and "testSecrets" in enabled_test_suite:
            for secret_info in enabled_test_suite["testSecrets"]:
                if secret := ConnectorTestContext._process_secret(
                    secret_info, secret_stores, raise_on_missing_secret_store, logger
                ):
    ...
    def _get_step_id_to_secret_mapping(
        self,
    ) -> Dict[CONNECTOR_TEST_STEP_ID, List[Secret]]:
        ...
        local_secrets = (
            self.local_secret_store.get_all_secrets() if self.local_secret_store else []
        )
        connector_test_suites_options = self.metadata.get(
    ...
    def _get_step_id_to_skip_according_to_metadata(
        self,
    ) -> List[CONNECTOR_TEST_STEP_ID]:
        ...
        enabled_test_suites = [
            option["suite"]
            for option in self.metadata.get("connectorTestSuitesOptions", [])
        ]
        return [
            step_id
            for test_suite_name, step_id in TEST_SUITE_NAME_TO_STEP_ID.items()
            if test_suite_name not in enabled_test_suites
        ]

    def _skip_metadata_disabled_test_suites(
        self, run_step_options: RunStepOptions
    ) -> RunStepOptions:
        ...
        if not run_step_options.keep_steps:
            run_step_options.skip_steps += (
                self._get_step_id_to_skip_according_to_metadata()
            )
