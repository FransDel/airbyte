...
class MigrateAirbyteLogger(Step):
    ...
    async def _run(self) -> StepResult:
        ...
        self.replace_text_in_files(
            file_path, "from airbyte_cdk import AirbyteLogger", "import logging"
        )
        self.replace_text_in_files(
            file_path, "from airbyte_cdk.logger import AirbyteLogger", "import logging"
        )
        self.replace_text_in_files(
            file_path, "logger: AirbyteLogger", "logger: logging.Logger"
        )
        self.replace_text_in_files(
            file_path, "AirbyteLogger()", 'logging.getLogger("airbyte")'
        )

        return StepResult(step=self, status=StepStatus.SUCCESS)

    def replace_text_in_files(
        self, root_dir: Path, old_text: str, new_text: str
    ) -> None:
...
async def run_connector_migrate_to_logging_logger_pipeline(
    ...
    steps_to_run.append(
        [
            StepToRun(
                id=CONNECTOR_TEST_STEP_ID.AIRBYTE_LOGGER_CANDIDATE,
