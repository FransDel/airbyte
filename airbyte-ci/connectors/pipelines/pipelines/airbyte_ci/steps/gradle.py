    ...
    async def _run(self, *args: Any, **kwargs: Any) -> StepResult:
        ...
        if self.context.s3_build_cache_access_key_id:
            gradle_container_base = gradle_container_base.with_secret_variable(
                ...
                self.context.s3_build_cache_access_key_id.as_dagger_secret(
                    self.dagger_client
                ),
            )
            if self.context.s3_build_cache_secret_key:
                gradle_container_base = gradle_container_base.with_secret_variable(
                    ...
                    self.context.s3_build_cache_secret_key.as_dagger_secret(
                        self.dagger_client
                    ),
                )
        ...
        if self.mount_connector_secrets:
            ...
            gradle_container = gradle_container.with_(
                await secrets.mounted_connector_secrets(
                    self.context, secrets_dir, self.secrets
                )
            )
