...
class UpdateBaseImageMetadata(Step):
    ...
    async def get_latest_base_image_address(self) -> Optional[str]:
        try:
            if (
                self.context.docker_hub_username is None
                or self.context.docker_hub_password is None
            ):
                raise ValueError(
                    "Docker Hub credentials are required to get the latest base image address"
                )
            version_registry_for_language = (
                await version_registry.get_registry_for_language(
                    ...
                    (
                        self.context.docker_hub_username.value,
                        self.context.docker_hub_password.value,
                    ),
                )
            )
    ...
    async def update_base_image_in_metadata(
        ...
        if not current_base_image_version_address:
            if set_if_not_exists:
                ...
                raise NoBaseImageAddressInMetadataError(
                    "No base image address found in metadata file"
                )
        else:
            new_raw_metadata = raw_metadata.replace(
                current_base_image_version_address, latest_base_image_version_address
            )
    ...
    async def _run(self) -> StepResult:
        ...
        current_base_image_address = self.context.connector.metadata.get(
