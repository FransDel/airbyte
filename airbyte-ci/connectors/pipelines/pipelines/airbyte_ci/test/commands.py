...
import asyncer
from pipelines.airbyte_ci.test import (
    INTERNAL_POETRY_PACKAGES,
    INTERNAL_POETRY_PACKAGES_PATH,
    pipeline,
)
...
async def find_modified_internal_packages(
...
async def get_packages_to_run(pipeline_context: ClickPipelineContext) -> Set[Path]:
    ...
    """
    if (
        not pipeline_context.params["poetry_package_paths"]
        and not pipeline_context.params["modified"]
    ):
    ...
    return poetry_package_paths.union(
        set(pipeline_context.params["poetry_package_paths"])
    )


def crash_on_any_failure(
...
@click.command()
@click.option(
    "--modified", default=False, is_flag=True, help="Run on modified internal packages."
)
@click.option(
    "--poetry-package-path",
    "-p",
...
async def test(pipeline_context: ClickPipelineContext) -> None:
    ...
    click.echo(
        f"Running poe tasks of the following packages: {', '.join([str(package_path) for package_path in poetry_package_paths])}"
    )
    dagger_client = await pipeline_context.get_dagger_client(
        pipeline_name="Internal poetry packages CI"
    )
    ...
    async with asyncer.create_task_group() as poetry_packages_task_group:
        for poetry_package_path in poetry_package_paths:
            poetry_package_poe_tasks_results.append(
                (
                    poetry_package_path,
                    poetry_packages_task_group.soonify(
