...
def parse_markdown(
    markdown_lines: list[str], github_repo: str
    ...
    for line_index, line in changelog_line_enumerator:
        ...
        if not changelog_entry_regexp or changelog_entry_regexp.group(
            "pr_number1"
        ) != changelog_entry_regexp.group("pr_number2"):
            break
        entry_version = semver.VersionInfo.parse(
            changelog_entry_regexp.group("version")
        )
        entry_date = datetime.datetime.strptime(
            changelog_entry_regexp.group("day"), "%Y-%m-%d"
        ).date()
        entry_pr_number = int(changelog_entry_regexp.group("pr_number1"))
        entry_comment = changelog_entry_regexp.group("comment")
        changelog_entry = ChangelogEntry(
            entry_date, entry_version, entry_pr_number, entry_comment
        )
...
class Changelog:
    def __init__(self, markdown: str, github_repo: str = AIRBYTE_GITHUB_REPO) -> None:
        ...
        self.changelog_entries_start_line_index, self.original_entries = parse_markdown(
            self.original_markdown_lines, github_repo
        )
    ...
    def add_entry(
        ...
        self.new_entries.add(
            ChangelogEntry(date, version, pull_request_number, comment)
        )
    ...
    def to_markdown(self) -> str:
        ...
        new_lines = (
            ...
            + self.original_markdown_lines[
                (self.changelog_entries_start_line_index + len(self.original_entries)) :
            ]
        )
        return "\n".join(new_lines) + "\n"
