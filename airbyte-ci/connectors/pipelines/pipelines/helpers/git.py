...
async def get_modified_files_in_branch_remote(
    current_git_repo_url: str,
    current_git_branch: str,
    current_git_revision: str,
    diffed_branch: str = "master",
    retries: int = 3,
) -> Set[str]:
        ...
        async with Connection(DAGGER_CONFIG) as dagger_client:
            ...
            modified_files = await container.with_exec(
                [
                    ...
                    "--name-only",
                    f"origin/{diffed_branch}...target/{current_git_branch}",
                ]
            ).stdout()
    except SessionError:
        if retries > 0:
            return await get_modified_files_in_branch_remote(
                current_git_repo_url,
                current_git_branch,
                current_git_revision,
                diffed_branch,
                retries - 1,
...
def get_modified_files_local(
    current_git_revision: str, diffed: str = "master"
) -> Set[str]:
...
async def get_modified_files_in_branch(
    ...
    if is_local:
        ...
        return await get_modified_files_in_branch_remote(
            current_repo_url, current_git_branch, current_git_revision, diffed_branch
        )


async def get_modified_files_in_commit_remote(
    current_git_branch: str, current_git_revision: str, retries: int = 3
) -> Set[str]:
    try:
        async with Connection(DAGGER_CONFIG) as dagger_client:
            container = await checked_out_git_container(
                dagger_client, current_git_branch, current_git_revision
            )
            modified_files = await container.with_exec(
                [
                    "diff-tree",
        ...
        if retries > 0:
            return await get_modified_files_in_commit_remote(
                current_git_branch, current_git_revision, retries - 1
            )
        else:
            raise
    return set(modified_files.split("\n"))
...
def get_modified_files_in_commit_local(current_git_revision: str) -> Set[str]:
    airbyte_repo = git.Repo()
    modified_files = airbyte_repo.git.diff_tree(
        "--no-commit-id", "--name-only", current_git_revision, "-r"
    ).split("\n")
    return set(modified_files)


async def get_modified_files_in_commit(
    current_git_branch: str, current_git_revision: str, is_local: bool = True
) -> Set[str]:
    if is_local:
        return get_modified_files_in_commit_local(current_git_revision)
    else:
        return await get_modified_files_in_commit_remote(
            current_git_branch, current_git_revision
        )
...
async def get_modified_files(
    ...
    """
    if ci_context is CIContext.MASTER or (
        ci_context is CIContext.MANUAL and git_branch == "master"
    ):
        return await get_modified_files_in_commit(git_branch, git_revision, is_local)
    return await get_modified_files_in_branch(
        git_repo_url, git_branch, git_revision, diffed_branch, is_local
    )
