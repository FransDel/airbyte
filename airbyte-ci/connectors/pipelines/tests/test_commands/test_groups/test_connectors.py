...
from pipelines.airbyte_ci.connectors.build_image import (
    commands as connectors_build_command,
)
from pipelines.airbyte_ci.connectors.publish import (
    commands as connectors_publish_command,
)
...
def test_get_selected_connectors_by_name_no_file_modification():
    ...
    selected_connectors = (
        connectors_commands.get_selected_connectors_with_modified_files(
...
def test_get_selected_connectors_with_modified():
    ...
    second_modified_connector = pick_a_random_connector(
        other_picked_connectors=[first_modified_connector]
    )
    modified_files = {
...
def test_get_selected_connectors_with_modified_and_language():
    ...
    assert len(selected_connectors) == 1
    assert (
        selected_connectors[0].technical_name
        == second_modified_connector.technical_name
    )
...
def test_get_selected_connectors_with_metadata_query():
    connector = pick_a_random_connector()
    metadata_query = (
        f"data.dockerRepository == '{connector.metadata['dockerRepository']}'"
    )
    selected_connectors = (
        connectors_commands.get_selected_connectors_with_modified_files(
            selected_names=(),
...
async def test_commands_do_not_override_connector_selection(
    ...
    mocker.patch.object(
        connectors_test_command, "ConnectorTestContext", mock_connector_context
    )
    mocker.patch.object(
        connectors_build_command, "ConnectorContext", mock_connector_context
    )
    mocker.patch.object(
        connectors_publish_command, "PublishConnectorContext", mock_connector_context
    )
    await runner.invoke(
        command, command_args, catch_exceptions=False, obj=click_context_obj
    )
    ...
    assert (
        mock_connector_context.call_args_list[0].kwargs["connector"]
        == selected_connector
    )
