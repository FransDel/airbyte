...
class TestBuildConnectorImage:
    ...
    def connector_with_base_image_no_build_customization(self, all_connectors):
        for connector in all_connectors:
            if connector.metadata and connector.metadata.get(
                "connectorBuildOptions", {}
            ).get("baseImage"):
                if not (connector.code_directory / "build_customization.py").exists():
                    return connector
        pytest.skip(
            "No connector with a connectorBuildOptions.baseImage metadata found"
        )

    @pytest.fixture
    def connector_with_base_image_with_build_customization(
        self, connector_with_base_image_no_build_customization
    ):
        dummy_build_customization = (
            Path(__file__).parent / "dummy_build_customization.py"
        ).read_text()
        (
            connector_with_base_image_no_build_customization.code_directory
            / "build_customization.py"
        ).write_text(dummy_build_customization)
    ...
    def connector_without_base_image(self, all_connectors):
        for connector in all_connectors:
            if connector.metadata and not connector.metadata.get(
                "connectorBuildOptions", {}
            ).get("baseImage"):
                return connector
        pytest.skip(
            "No connector without a connectorBuildOptions.baseImage metadata found"
        )

    @pytest.fixture
    def test_context_with_real_connector_without_base_image(
        self, connector_without_base_image, dagger_client, current_platform
    ):
        context = ConnectorContext(
            pipeline_name="test build",
    ...
    async def test__run_using_base_image_with_mocks(
        self, mocker, test_context_with_connector_with_base_image, all_platforms
    ):
        ...
        step = python_connectors.BuildConnectorImages(
            test_context_with_connector_with_base_image
        )
    ...
    @pytest.mark.slow
    async def test_building_from_base_image_for_real(
        self, test_context_with_real_connector_using_base_image, current_platform
    ):
        step = python_connectors.BuildConnectorImages(
        ...
        assert await built_container.entrypoint() == build_customization.get_entrypoint(
            step.context.connector
        )
        assert (
            await built_container.label("io.airbyte.version")
            == test_context_with_real_connector_using_base_image.connector.metadata[
                "dockerImageTag"
            ]
        )
        assert (
    ...
    async def test_building_from_base_image_with_customization_for_real(
        ...
        built_container = step_result.output[current_platform]
        assert (
            await built_container.env_variable("MY_PRE_BUILD_ENV_VAR")
            == "my_pre_build_env_var_value"
        )
        assert (
            await built_container.env_variable("MY_POST_BUILD_ENV_VAR")
            == "my_post_build_env_var_value"
        )

    async def test__run_using_base_dockerfile_with_mocks(
        self, mocker, test_context_with_connector_without_base_image, all_platforms
    ):
        ...
        step = python_connectors.BuildConnectorImages(
            test_context_with_connector_without_base_image
        )
        step_result = await step._run()
    ...
    async def test_building_from_dockerfile_for_real(
        self, test_context_with_real_connector_without_base_image
    ):
        step = python_connectors.BuildConnectorImages(
            test_context_with_real_connector_without_base_image
        )
        step_result = await step._run()
        step_result.status is StepStatus.SUCCESS
