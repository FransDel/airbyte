...
class TestAcceptanceTests:
    @staticmethod
    def get_dummy_cat_container(
        dagger_client: dagger.Client,
        exit_code: int,
        secret_file_paths: List,
        stdout: str,
        stderr: str,
    ):
        ...
        container = (
            ...
            .from_("bash:latest")
            .with_exec(
                ["mkdir", "-p", common.AcceptanceTests.CONTAINER_TEST_INPUT_DIRECTORY]
            )
            .with_exec(
                ["mkdir", "-p", common.AcceptanceTests.CONTAINER_SECRETS_DIRECTORY]
            )
        )
        ...
        for secret_file_path in secret_file_paths:
            ...
            container = container.with_exec(
                ["sh", "-c", f"echo foo > {secret_file_path}"]
            )
        return container.with_new_file(
            "/stupid_bash_script.sh",
            contents=f"echo {stdout}; echo {stderr} >&2; exit {exit_code}",
        )
    ...
    @pytest.fixture
    def another_dummy_connector_under_test_container(
        self, dagger_client
    ) -> dagger.File:
        return dagger_client.container().from_("airbyte/source-pokeapi:latest")

    async def test_skipped_when_no_acceptance_test_config(
        self, mocker, test_context_ci
    ):
    ...
    async def test__run(
        ...
        mocker.patch.object(
            common.AcceptanceTests,
            "_build_connector_acceptance_test",
            side_effect=async_mock,
        )
        mocker.patch.object(
            common.AcceptanceTests,
            "get_cat_command",
            return_value=["bash", "/stupid_bash_script.sh"],
        )
        test_context_ci.get_connector_dir = mocker.AsyncMock(
            return_value=test_input_dir
        )
        acceptance_test_step = common.AcceptanceTests(test_context_ci, secrets=[])
        step_result = await acceptance_test_step._run(None)
        ...
        if expect_updated_secrets:
            assert (
                ...
                == await cat_container.directory(
                    f"{common.AcceptanceTests.CONTAINER_SECRETS_DIRECTORY}"
                ).entries()
            )
            assert any(
                "updated_configurations" in str(file_name)
                for file_name in await test_context_ci.updated_secrets_dir.entries()
            )
    ...
    def test_input_dir(self, dagger_client, tmpdir):
        with open(tmpdir / "acceptance-test-config.yml", "w") as f:
            yaml.safe_dump(
                {"connector_image": "airbyte/connector_under_test_image:dev"}, f
            )
    ...
    def get_patched_acceptance_test_step(self, mocker, test_context_ci, test_input_dir):
        ...
        secrets = [
            Secret("config.json", in_memory_secret_store, file_name="config.json")
        ]
        test_context_ci.get_connector_dir = mocker.AsyncMock(
            return_value=test_input_dir
        )
        ...
        mocker.patch.object(
            docker, "load_image_to_docker_host", return_value="image_sha"
        )
        mocker.patch.object(
            docker, "with_bound_docker_host", lambda _, cat_container: cat_container
        )
    ...
    async def test_cat_container_provisioning(
        ...
        acceptance_test_step = self.get_patched_acceptance_test_step(
            mocker, test_context_ci, test_input_dir
        )
        ...
        assert (
            await cat_container.with_exec(["pwd"]).stdout()
        ).strip() == acceptance_test_step.CONTAINER_TEST_INPUT_DIRECTORY
        test_input_ls_result = await cat_container.with_exec(["ls"]).stdout()
        assert all(
            file_or_directory in test_input_ls_result.splitlines()
            for file_or_directory in ["secrets", "acceptance-test-config.yml"]
        )
        assert (
            await cat_container.with_exec(
                [
                    "cat",
                    f"{acceptance_test_step.CONTAINER_SECRETS_DIRECTORY}/config.json",
                ]
            ).stdout()
            == "***"
        )
        env_vars = {
            await env_var.name(): await env_var.value()
            for env_var in await cat_container.env_variables()
        }
    ...
    async def test_cat_container_caching(
        ...
        initial_datetime = datetime.datetime(
            year=1992, month=6, day=19, hour=13, minute=1, second=0
        )
        ...
        with freeze_time(initial_datetime) as frozen_datetime:
            ...
            first_cat_container = (
                await acceptance_test_step._build_connector_acceptance_test(
                    dummy_connector_under_test_container, test_input_dir
                )
            )
    ...
    async def test_params(self, dagger_client, mocker, test_context_ci, test_input_dir):
        ...
        assert set(acceptance_test_step.params_as_cli_options) == {
            "-ra",
            "--disable-warnings",
            "--durations=3",
        }
