...
from pipelines.airbyte_ci.connectors.test.steps.python_connectors import (
    PyAirbyteValidation,
    UnitTests,
)
...
class TestUnitTests:
    ...
    def certified_connector_with_setup(self, all_connectors):
        for connector in all_connectors:
            if connector.support_level == "certified" and connector.language in [
    ...
    def context_for_certified_connector_with_setup(
        self, mocker, certified_connector_with_setup, dagger_client, current_platform
    ):
    ...
    @pytest.fixture
    async def certified_container_with_setup(
        self, context_for_certified_connector_with_setup, current_platform
    ):
        result = await BuildConnectorImages(
            context_for_certified_connector_with_setup
        ).run()
        return result.output[current_platform]

    @pytest.fixture
    def context_for_connector_with_poetry(
        self, mocker, connector_with_poetry, dagger_client, current_platform
    ):
        context = ConnectorContext(
            pipeline_name="test unit tests",
    ...
    @pytest.fixture
    async def container_with_poetry(
        self, context_for_connector_with_poetry, current_platform
    ):
        result = await BuildConnectorImages(context_for_connector_with_poetry).run()
        return result.output[current_platform]

    async def test__run_for_setup_py(
        self, context_for_certified_connector_with_setup, certified_container_with_setup
    ):
        # Assume that the tests directory is available
        result = await UnitTests(context_for_certified_connector_with_setup)._run(
            certified_container_with_setup
        )
        assert isinstance(result, StepResult)
        assert (
            "test session starts" in result.stdout
            or "test session starts" in result.stderr
        )
        assert (
        ...
        assert (
            "Required test coverage of" in result.stdout
        ), "A test coverage threshold should be defined for certified connectors."
        pip_freeze_output = await result.output.with_exec(
            ["pip", "freeze"], skip_entrypoint=True
        ).stdout()
        assert (
            context_for_certified_connector_with_setup.connector.technical_name
            in pip_freeze_output
        ), "The connector should be installed in the test environment."
        assert (
            "pytest" in pip_freeze_output
        ), "The pytest package should be installed in the test environment."
        assert (
            "pytest-cov" in pip_freeze_output
        ), "The pytest-cov package should be installed in the test environment."

    async def test__run_for_poetry(
        self, context_for_connector_with_poetry, container_with_poetry
    ):
        # Assume that the tests directory is available
        result = await UnitTests(context_for_connector_with_poetry).run(
            container_with_poetry
        )
        assert isinstance(result, StepResult)
        ...
        assert (
            "test session starts" in result.stdout
            or "test session starts" in result.stderr
        ), "The pytest tests should have started."
        pip_freeze_output = await result.output.with_exec(
            ["poetry", "run", "pip", "freeze"], skip_entrypoint=True
        ).stdout()

        assert (
            context_for_connector_with_poetry.connector.technical_name
            in pip_freeze_output
        ), "The connector should be installed in the test environment."
        assert (
            "pytest" in pip_freeze_output
...
class TestPyAirbyteValidationTests:
    ...
    @pytest.fixture
    def context_for_valid_connector(
        self, compatible_connector, dagger_client, current_platform
    ):
        context = ConnectorContext(
    ...
    @pytest.fixture
    def context_for_invalid_connector(
        self, incompatible_connector, dagger_client, current_platform
    ):
        context = ConnectorContext(
            pipeline_name="test pyairbyte validation",
    ...
    async def test__run_validation_success(
        self, mocker, context_for_valid_connector: ConnectorContext
    ):
        result = await PyAirbyteValidation(context_for_valid_connector)._run(
            mocker.MagicMock()
        )
        assert isinstance(result, StepResult)
    ...
    async def test__run_validation_skip_unpublished_connector(
        ...
        result = await PyAirbyteValidation(context_for_invalid_connector)._run(
            mocker.MagicMock()
        )
        assert isinstance(result, StepResult)
    ...
    async def test__run_validation_fail(
        ...
        metadata["remoteRegistries"] = {
            "pypi": {"enabled": True, "packageName": "airbyte-source-postgres"}
        }
