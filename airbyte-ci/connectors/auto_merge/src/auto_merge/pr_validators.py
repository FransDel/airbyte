...
def only_modifies_connectors(
    head_commit: GithubCommit, pr: PullRequest, required_checks: set[str]
) -> Tuple[bool, Optional[str]]:
    modified_files = pr.get_files()
    for file in modified_files:
        if not any(
            file.filename.startswith(prefix) for prefix in CONNECTOR_PATH_PREFIXES
        ):
...
def head_commit_passes_all_required_checks(
    ...
    successful_status_contexts = [
        commit_status.context
        for commit_status in head_commit.get_statuses()
        if commit_status.state == "success"
    ]
